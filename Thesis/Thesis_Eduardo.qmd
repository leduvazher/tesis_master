...
/* Espacio adicional para la leyenda */
.plot-container {
...
ph <- autoplot(window(var_model[,3], start = c(2006,1), end = c(2020,3), frequency = 12), ylab = "Male Homicides") +
  ...
  theme(
    ...
    legend.position = "none" # Quitamos la leyenda de ggplot para usar solo la de plotly
  )

# Convertir a plotly con leyenda interna
ggplotly(ph, width = 800, height = 400) %>% 
  layout(
    autosize = TRUE,
    margin = list(l = 50, r = 20, b = 80, t = 50), # Más espacio en la parte inferior
    legend = list(
      orientation = "h",      # Horizontal
      xanchor = "center",     # Centrada
      yanchor = "top",        # Anclada arriba
      x = 0.5,                # Centro horizontal
      y = -0.2                # Posición vertical (ajustada específicamente para Posit Connect)
    ),
    annotations = list(
      list(
        text = "Time",
  ...
  config(displayModeBar = FALSE) # Ocultar barra de herramientas
```
